...
encode str = chunkList sqr str'
  where str' = concat . words $ (filter (isLetter) str)
        sqr  = trace (show "debuggggg") (squareit $ length str')   -- <------
```

result:
```hs
ghci> encode "If man was meant to stay on the ground, god would have given us roots."
[""debuggggg"
Ifmanwas","meanttos","tayonthe","groundgo","dwouldha","vegivenu","sroots"]
```

The first argument is printed while the second is returned (normally).
...
encodeSquare str = trace (str') (chunkList sqr $ addSpaces (sqr `mod` (length str')) str')
  where str' = concat . words $ (filter (isNumberLetter) str)
        sqr  = trace (show (squareit $ length str')) (squareit $ length str')
