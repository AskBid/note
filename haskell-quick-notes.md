...
like functor but instead of only one function, you actually have a container of functions too, that gets applied to all elements.
...
A `Monad` needs to be an `Applicative` before being a `Monad`.
...
And an `Applicative` needs to be a `Functor` before being an `Applicative`.
...
And here is a `Functor`, the building block.
...
Functors aren't to be considered first as a iteraror, is most and foremost a wrapper, iterator as array is only once case of it where the wrapper is a `[]`.

The most simple type of functor fmap instance could be with identity as such:
```haskell
data Maybe a = Just a | Nothing

data Identity a = Identity a --a wrapper

fmapForidentity :: (a -> b) -> Identity a -> Identity b
fmapForidentity func (Identity x) = Identity $ func x
```

>Notice how `Maybe` is non other (`Just` ;] ) than an identity type with the option of being also `Nothing` 

`fmap` (or `<$>`) is just a way to apply a function to a value wrapped by the wrapper.
...
if you where to try `instance Functor Int where` it would not work because as specified in the the typeclass Functor must have kind `* -> *` (a schema for wrapper really), while `Int` has kind `*` only.

But what if we need to instantiate `Functor` for `Either` which is of kind `* -> * -> *`?

What would the right instantiation be life:
```haskell
...
It starts to emerge that Functor is much more about what the isntantiated type is than what a functor itself is.
...
If we compare the two main funtions of functor (`fmap` = `<$>`) and applicative (`<*>`) we discover that `<*>` isn't just that another slighter more complicated `fmap`, where also the function is wrapped:
```haskell
fmap  ::   (a -> b) -> f a -> f b
(<*>) :: f (a -> b) -> f a -> f b 
```
...
## `sequenceA`
```haskell
sequenceA :: (Traversable t, Applicative a) => t (f a) -> f (t a)
```
...
> sequenceA $ sequenceA [Just 1, Just 2, Just 3]
...
You can think of sequence as a way of collecting results.

### `sequence`

`sequence` is exactly like `sequenceA` only that one is for `Monads` while the second one is for `Applicative`.

```haskell
sequenceA :: (Traversable t, Applicative a) => t (f a) -> f (t a)
--Vs
sequence :: (Traversable t, Monad m) => t (f m) -> f (t m)
```
...
# Precedences Functions and Operators

In Haskell, operator precedence determines the order in which operations are performed when an expression contains multiple operators. The phrase "binds tighter" refers to an operator having higher precedence, which means it binds to its operands more closely.

In the context of your error message, it's relevant to understand that function application has the highest precedence in Haskell. So, when you write an expression like f x + g y, it's interpreted as (f x) + (g y) because function application binds tighter than addition.

On the other hand, the $ operator has very low precedence, so it's often used to avoid the need for parentheses. For example, f $ g x is equivalent to f (g x).

In your original code:

haskell
Copy code
| even n    = (+1) <$> collatz $ divInteger n 2
| otherwise = (+1) <$> collatz $ (+1) $ n * 3
The $ operator is used, but it has lower precedence than function application (<$>). This caused the expression (+1) <$> collatz to be applied to divInteger n 2 or (+1) $ n * 3 instead of being applied to the entire collatz expression.

By adding parentheses as suggested in the correction, you ensure that the composition is applied to the result of divInteger n 2 or (+1) $ n * 3 before being passed as an argument to collatz.

```
ghci> :{
ghci| collatz :: Integer -> Maybe Integer
ghci| collatz 1 = Just 0
ghci| collatz _ = Nothing
ghci| :}
ghci> :t collatz
collatz :: Integer -> Maybe Integer
ghci> :t ((+1) <$> collatz 1)
((+1) <$> collatz 1) :: Maybe Integer
ghci> :t ((+1) <$> collatz $ 1 + 1)

<interactive>:1:3: error: [GHC-39999]
    • No instance for ‘Num (Maybe Integer)’ arising from a use of ‘+’
    • In the expression: (+)
      In the first argument of ‘(<$>)’, namely ‘(+ 1)’
      In the first argument of ‘($)’, namely ‘(+ 1) <$> collatz’

ghci> :t ((+1) <$> (collatz $ 1 + 1))
((+1) <$> (collatz $ 1 + 1)) :: Maybe Integer
